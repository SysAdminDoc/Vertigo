...
- **Worker-output scrub parity.** Subtitle sidecars now write through an
  ...
  siblings on cancel or exception. Existing caption sidecars remain intact.
  Highlight and auto-editor workers were audited: they return in-memory
  candidates, and auto-editor already keeps its JSON export in a temporary
  directory.
...
| Full OBS auto-highlight plugin | Zero OSS precedent worth forking; not a Vertigo fit. |
...
- Remotion-style declarative video render as an export backend for burned overlays
